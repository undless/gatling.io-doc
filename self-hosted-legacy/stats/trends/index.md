...
Runs list and trends can be accessed by clicking on the {{< icon history >}} icon in the [simulations table]({{< ref "/execute/simulations#simulations-table" >}}).
...
Like the result of the latest run in the [simulations table]({{< ref "/execute/simulations#simulations-table" >}}) you have access to the [logs]({{< ref "/execute/simulations#logs" >}}) of the run by clicking on the {{< icon file-alt >}} icon and you can sort the table by each columns. The logs are only available for run which are not flagged as "Successful".
